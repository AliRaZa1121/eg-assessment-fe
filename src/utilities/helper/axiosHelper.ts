...
if (localStorage.getItem(LOCAL_STORAGE_KEYS.AUTH_TOKEN)) {
    const { accessToken } = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEYS.AUTH_TOKEN) || "{}");
...
httpService.interceptors.response.use(undefined, async (error: AxiosError<ApiErrorResponse>) => {
    ...
    if (error?.response?.status === 401) {
        const { refreshToken } = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEYS.AUTH_TOKEN) || "{}");
